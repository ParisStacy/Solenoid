...
public class PlayerControl : MonoBehaviour {
    ...
    void Update() {
        ...
        //Condition checks for grounded / crouching
        RaycastHit hit;
...
//            _moveDirection += Vector3.Cross(slopeHit.normal, slopeHit.transform.forward) * _speed;
    ...
    //Behavior when hanging on ledge (void)
    void hangingUpdate() {

    }

    //Behavior when mantling (void)
    void climbingUpdate() {
    ...
    //SUMMARY: determing player camera's target offset for things like crouching, jumping, etc during animations
